...
# Negated Literals
Unlike other QUBO/HUBO tools, QUBO++ natively supports negated literals of binary variables.
...
QUBO++ can handle such terms without expanding negated literals.

## Using negated literals in QUBO++

QUBO++ uses the `~` operator to represent negated literals,
...
The solvers bundled with QUBO++ accept HUBO formulas with negated literals directly,
...
QUBO++ can outperform conventional tools.
...
The above QUBO++ program demonstrates this behavior at different variable counts.
...
int main() {
  ...
  auto f = x * y * z + ~x * ~y * ~z;
  std::cout << "f = " << f << std::endl;
  std::cout << "f(0, 0, 0) = " << f({{x, 0}, {~y, 1}, {~z, 1}}) << std::endl;
}
```
{% endraw %}
This program produces the following output:
...
# 否定リテラル
他のQUBO/HUBOツールとは異なり、QUBO++はバイナリ変数の否定リテラルをネイティブにサポートしています。
...
QUBO++はこのような項を否定リテラルを展開せずに扱うことができます。

## QUBO++での否定リテラルの使用

QUBO++では `~` 演算子で否定リテラルを表現します。
`~x` は `x` の否定リテラルを表します。
...
QUBO++に付属するソルバーは、否定リテラルを含むHUBO式を正リテラルに展開せずに直接受け付けます。
一方、従来のツールは $\bar{x}$ の代わりに $1-x$ を使用する必要があります。
そのため、多くの否定リテラルを含む項を持つHUBOモデルでは、QUBO++は従来のツールより高い性能を発揮できます。
...
上記のQUBO++プログラムは、変数の数に応じて以下のように動作します。
