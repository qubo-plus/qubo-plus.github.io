// QUBO++ User Portal SPA — vanilla JS + AWS Amplify JS v6 (Auth category).
...
// the default token provider. Bump all three URLs together (docs/portal/README.md).
...
function main() {
  ...
  async function renderTopnav() {
    ...
    if (!(await isSignedIn())) {
      ...
      sub.textContent = `Create your ${window.QBPP_BRAND || "QUBO++"} account to get a free Trial License you can start anytime.`;
      return;
    }
    try {
  ...
  function fmtExpiry(item) {
    ...
    // Portal trials are dormant until the user clicks Start; the clock begins
    // then, not on first activate.
    if (item.source === "portal_trial") {
      const days = (item.expiry_days && item.expiry_days > 0) ? item.expiry_days : 30;
      return `Not started — click Start/Renew to begin/extend your ${days}-day trial`;
    }
  ...
  function isActive(lic) {
    ...
    // A portal trial that hasn't been started yet is not active.
  ...
  function renderLicense(lic, acts, actsError) {
    ...
    let renewBadge = "";
    if (isPortalTrial) {
      // ONE button labelled "Start/Renew" in every state — both actions call
      // /me/trial/renew, and always showing both words makes users aware the
      // trial can be renewed (not just started). Dormant (never started) →
      // begins the clock, always enabled. Started → extends it, gated to the
      // renewal window (mirroring the server check).
      const now = Math.floor(Date.now() / 1000);
      const started = !!(lic.expiry && lic.expiry > 0);
      const expired = started && lic.expiry < now;
      const avail = Number(lic.renew_available_at || 0);
      const label = "Start/Renew";
      const allowed = !started || expired || !avail || now >= avail;
      const attrs = `data-renew="${escapeHTML(lic.license_key)}"`;
      if (allowed) {
        // Actionable — solid amber + pulsing ring (.renew-ready in style.css).
        renewBtn = `<button type="button" class="renew-ready" ${attrs} title="Start your trial, or extend it later — keeps the same license key">${label}</button>`;
        renewBadge = ` <span class="renew-badge">Start/Renew available</span>`;
      } else {
        const dt = fmtDateTime(avail);
        renewBtn = `<button type="button" class="secondary" ${attrs} disabled title="Start/Renew available from ${escapeHTML(dt)}">${label}</button>`;
      }
    }
  ...
  async function renewLicense(btn) {
    if (!confirm(
      "Start or renew your Trial License now? This sets a new expiry date " +
      "(a not-yet-started trial begins now; an active one is extended). The " +
      "license key and any existing machine activations stay as they are. " +
      "Continue?"
    )) return;
    btn.disabled = true;
    ...
    try {
      ...
      alert((data.started ? "Trial started. Expires: " : "Trial extended. New expiry: ") + dt);
    } catch (e) {
      btn.disabled = false;
      btn.textContent = orig;
      alert("Could not start/renew: " + e.message);
    }
  }
  ...
  $("#delete-form").addEventListener("submit", async (ev) => {
    ...
    if (!confirm(
      "Delete your account?\n\n" +
      "ALL of your licenses will STOP WORKING " +
