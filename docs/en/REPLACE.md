...
This program finds a partition of the numbers in the following array **`w`** into two subsets $P$ and $Q$ ($=\overline{P}$) such that the difference between their sums is minimized:
```cpp
...
[Native integer variables](NATIVE_INTEGER) (`qbpp::int_var()`) can be replaced by constants or expressions in exactly the same way.
Substitution is a purely symbolic rewrite: the declared range of the variable is neither checked nor carried over to the result.
See [Native integer variables](NATIVE_INTEGER) for details and an example.
