...
The call to `search()` returns a `qbpp::EasySolverSol` object named `sols`, which stores the best solutions.
Since `qbpp::EasySolverSol` provides iterator access to the stored best-energy solutions (`begin()`, `end()`, `cbegin()`, and `cend()`), they can be printed using a range-based for loop.
