...
characteristic of this page. The constraint-programming
...
int main() {
  ...
  for (size_t i = 0; i < w.size(); ++i) { p += w[i] * x[i]; q += w[i] * ~x[i]; }
  auto f = qbpp::sqr(p - q);
...
> `topk_sols` is best-effort: these solvers
...
The callback API is identical to `qbpp::ABS3Solver`, and there are two ways to use it:

1. **Register a function** — `solver.set_callback(fn)`. `fn` receives the solver itself (no inheritance)
2. **Subclass** — derive from the solver and override the `callback()` virtual method

If both are used, the override wins. Call the base class `callback()` from the override to run both.
...
int main() {
  ...
  qbpp::HighsSolver solver(f);
  solver.set_callback([](const qbpp::HighsSolver& s) {
    if (s.event() == qbpp::CallbackEvent::BestUpdated) {
      std::cout << "New best: energy=" << s.best_sol().energy()
                << "  bound=" << s.bound() << std::endl;
      if (s.best_sol().energy() == 0) s.terminate();
    }
  });

  auto sol = solver.search({{"time_limit", 5}});
  std::cout << "energy=" << sol.energy() << std::endl;
}
```
{% endraw %}
...
```sh
g++ -std=c++17 -Wfloat-conversion your_program.cpp -o your_program \
