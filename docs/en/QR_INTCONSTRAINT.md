...
The integer variables on this page are the ones expanded into binary
variables. [Native integer variables](NATIVE_INTEGER) (`qbpp::IntVar`),
which keep the integer value as is, are a distinct type from
`qbpp::Expr`; their reference is in [Variables and Expressions](QR_VARIABLE).
Both kinds can appear in the body of a `cons()` constraint.
