...
- **Native integer variables** — Bounded integer variables are searched directly as integers, never expanded into binary variables. There is no limit on the bit width of the range (combined with arbitrary-precision integers, ranges beyond $10^{20}$ work), and they are passed to MIP solvers as integer variables as is. See [Native Integer Variables](NATIVE_INTEGER) ([Python version](python/NATIVE_INTEGER)) for details.
- **Nonlinear functions** — Absolute value `abs`, `relu`, `max`, and `min` can be used directly inside expressions. The bundled solvers handle the function values directly — no auxiliary variables or penalty polynomials to design. See [Nonlinear Functions and Native Constraints](CONSTRAINTS) ([Python version](python/CONSTRAINTS)) for details.
...
- **Massive variable capacity** — A single model can use up to **1,073,741,823** ($2^{30}-1$) binary variables and as many integer variables.
...
- **Experimental third-party solver support** — Call SCIP, HiGHS, GLPK, CBC, IBM Qiskit Optimization, dimod ExactSolver, the D-Wave samplers (Neal / Tabu / Steepest), OpenJij, TYTAN-SDK MIKAS, qubovert, Simulated Bifurcation, and Google OR-Tools CP-SAT through a unified `Solver.search()` protocol. SCIP, HiGHS, GLPK, and CBC are available from C++ (QUBO++) as well; the others are available from PyQBPP. See [MILP Solvers](MILP_SOLVERS) and [CP Solvers](CP_SOLVERS).
...
### **ABS3 Supported GPU architectures**
  - **sm_75** : NVIDIA T4, Quadro RTX 6000/8000, GeForce RTX 2080 Ti/2080/2070/2060 (Turing)
...
**QUBO++ is not limited to Ubuntu 20.04**; it has also been tested on Ubuntu 22.04/24.04/26.04 and other Linux distributions (including CentOS/RHEL-based systems).
...
- **[QUBO/HUBO Solvers](python/QUBO_HUBO_SOLVERS)** — the model is taken directly (OpenJij, dimod samplers, …; PyQBPP only).
...
Third-party solvers are limited to the open-source ones listed below, whose
license terms are clear. None of them is bundled with QUBO++; each must be
...
These integrations are **experimental** — their APIs may change without notice.
See each page above for the full solver list, installation, and per-backend
caveats.
