...
```bash
g++ test.cpp -o test -std=c++17 -Wfloat-conversion -ldl -pthread
```
This command creates an executable file named test.
The compiler options mean the following:
- **`-std=c++17`**: Use the C++17 standard.
- **`-Wfloat-conversion`**: Warn about implicit float-to-integer conversions. With the integer-coefficient library a real literal such as `3.5 * x` or `f <= 2.5` is silently truncated (to `3 * x` and `f <= 2`), so this flag is recommended. Use the `#define DOUBLE_TYPE` build if you need real coefficients.
