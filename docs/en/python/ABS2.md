...
The `gpu` constructor argument selects how many GPUs to use
...
If you do both, the override wins. Call `super().callback()` from the
...
Inside a callback you can use `best_sol()`, `event()`, `timer(seconds)`,
`hint(sol)` and `terminate()`. Timer events are off by default, so
`timer()` is usually called once inside `Start`.
