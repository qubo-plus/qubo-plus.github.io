...
f = 1 +2*x*y -x +2*y
```
In most cases, there is no need to explicitly use product terms.
...
If you need an explicit empty `Expr`, use the factory function `qbpp.expr()` (the Python counterpart of C++ `qbpp::toExpr(0)`).
