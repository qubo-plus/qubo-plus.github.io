...
The basic operators **`+`**, **`-`**, and **`*`** work for arrays of variables and expressions. **`/`** (by an integer) works for arrays of expressions.
...
Similarly, the compound operators **`+=`**, **`-=`**, **`*=`**, and **`/=`** work for arrays of expressions.
