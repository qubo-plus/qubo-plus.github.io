...
For more control there are two ways, both offering the same events and the same methods:

1. **Register a function** — `solver.set_callback(fn)`. `fn` is called as `fn(solver)`, receiving the solver itself. No inheritance is involved, so use this when the callback has to live in an existing class hierarchy or alongside a mixin
2. **Subclass** — derive from `ABS3Solver` and override the `callback()` method (no arguments)

If both are used, the override wins and the registered function is not called. Call `super().callback()` from the override to run both.
...
Inside `callback()` (or on the solver `s` handed to a registered function), the following methods are available:
...
### Method 1: Registering a Function
...
def on_event(s):
    if s.event() == 0:        # Start
        s.timer(1.0)          # enable a timer callback every second
    if s.event() == 1:        # BestUpdated
        sol = s.best_sol()
...
Any callable works, not just a plain function — an instance of a class with
`__call__` is convenient when the callback needs state:
...
class BestWatcher:
    ...
    def __call__(self, s):
        if s.event() == 1:                        # BestUpdated
            self.history.append(s.best_sol().energy)

watcher = BestWatcher()
solver = qbpp.ABS3Solver(f)
solver.set_callback(watcher)
sol = solver.search(time_limit=5)
print(watcher.history)
```

Call `solver.set_callback(None)` to unregister.
