...
variables. [Native integer variables](NATIVE_INTEGER) (`IntVar`), which
keep the integer value as is, are a distinct type from `Expr`; their
reference is in [Variables and Expressions](QR_VARIABLE).
