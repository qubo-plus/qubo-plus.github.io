...
- **`qbpp.abs(f)`** / **`qbpp.abs(f, k)`**:
  Returns an `Expr` denoting $|f|$ / $|f|^k$ (`k` is 1 or 2).

- **`qbpp.relu(f)`** / **`qbpp.relu(f, k)`**:
  Returns an `Expr` denoting $\max(0, f)$ / $\max(0, f)^k$
  (`k` is 1 or 2).

- **`qbpp.max(f, g)`** / **`qbpp.min(f, g)`**:
  Returns an `Expr` denoting the maximum / minimum of two expressions.
...
- Any constant coefficient works, including negative ones (a negative
  coefficient rewards a large function value).
- `expand_cons()`, `reduce()`, and `binarize()` are not supported.
- The bundled solvers support all of these functions. The MIP wrappers
  do not support nonlinear functions (only the C++ ScipSolver Quadratic
  formulation does).

> **Note**: `qbpp.abs` / `qbpp.max` / `qbpp.min` are distinct from the
> Python built-ins `abs` / `max` / `min`. Watch out for name collisions
> when using `from pyqbpp import *`.
