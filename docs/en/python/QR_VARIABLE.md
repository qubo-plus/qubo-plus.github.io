...
| `import pyqbpp.dc64e128` | `float` | `float` | 128-bit integer solver (higher precision, recommended) |
| `import pyqbpp.dc128e128` | `float` | `float` | 128-bit integer solver (widest range) |
...
- Dyadic coefficients (1, 1/2, 1/4, ...) are represented exactly. A coefficient vastly smaller
  ...
  notice; `pyqbpp.dc64e128` gives a much wider dynamic range.
...
## Integer variables
QUBO++ provides two kinds of integer variables.

| Kind | Declaration | Internal representation | When to use |
|---|---|---|---|
| Integer variable | `qbpp.var("x", between=(l, u))` | expanded into binary variables | can be handed to external binary-only solvers |
| [Native integer variable](NATIVE_INTEGER) | `qbpp.var("x", integer=(l, u))` | keeps the integer value as is | quantities solved by the bundled and MIP solvers |
...
A **native integer variable** is an `IntVar` object that keeps its integer
value as is. It is a distinct type rather than an `Expr`, but it is
converted to `Expr` automatically when used in an expression, so it reads
just like an ordinary variable. See
[Native Integer Variables](NATIVE_INTEGER) for details.

### Native integer variable creation functions

- **`qbpp.var("name", integer=(l, u))`**:
  Creates a native integer variable named `"name"` with the range
  `[l, u]`. The range is mandatory and must satisfy `l < u`.

- **`qbpp.var("name", s1, s2, ..., integer=(l, u))`**:
  Creates an array of native integer variables sharing the range `[l, u]`.
  Elements are named `name[i]` / `name[i][j]` and accessed with `x[i]` /
  `x[i, j]` (`x(i, j)` also works). The `shape=` keyword is accepted too.

### Native integer variable properties

- **`x.lo`** (property): Returns the minimum value `l` of `x`.
- **`x.hi`** (property): Returns the maximum value `u` of `x`.

An array `s` provides `s.shape` / `s.ndim` / `s.total` and `s.sum()`,
which returns the sum of all elements.

The value in a solution `sol` is read with `sol(x)` and written with
`sol.set(x, v)`.

### Conversion

- **`qbpp.binarize(f)`**:
  Returns a new `Expr` in which the native integer variables of `f` are
  expanded into the binary encoding. MIP solvers handle integer variables
  directly, so this is needed only when handing a model to an external
  QUBO solver that accepts binary variables only.
