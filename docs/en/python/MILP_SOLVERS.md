...
quadratic objective **directly** do **not**
...
see [QUBO/HUBO Solvers](QUBO_HUBO_SOLVERS).
...
1. **Register a function** — `solver.set_callback(fn)`. `fn` is called as `fn(solver)`, receiving the solver itself (no inheritance)
2. **Subclass** — derive from the solver and override `callback()`

If both are used, the override wins. Call `super().callback()` from the override to run both.

Either way, `event()`, `best_sol()` (a `pyqbpp.Sol`), `bound()`, `timer(seconds)`,
and `terminate()` are available. The event constants are class attributes:
...
def on_event(s):
    if s.event() == s.EVENT_BEST_UPDATED:
        ...
        if best.energy == 0:
            s.terminate()             # stop as soon as the optimum is found
