...
Collecting these forced values into a `qbpp::MapList` and passing it to `qbpp::replace` removes the corresponding variables from the QUBO expression, drastically reducing the number of variables the solver needs to handle.

## QUBO++ program

The following QUBO++ program builds the QUBO expression from the constraints, fixes the clue-related variables, and then solves the puzzle with EasySolver:
