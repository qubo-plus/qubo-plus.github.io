...
From **C++**, the only external QUBO/HUBO solver currently wrapped is the
**[Gurobi Optimizer](https://www.gurobi.com)** (an exact MIQP solver that
accepts the quadratic objective directly and returns a proven optimum with a
dual bound). The much larger lineup of heuristic samplers and annealers
(Fixstars Amplify, D-Wave, OpenJij, TYTAN-SDK, qubovert, Simulated Bifurcation)
and other exact backends (IBM CPLEX, dimod, Qiskit) is available **only from
PyQBPP (Python)** — see the [PyQBPP QUBO/HUBO Solvers page](python/QUBO_HUBO_SOLVERS).
...
- **`info(key)`** — solver info strings (see below)
