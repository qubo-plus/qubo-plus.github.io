...
std::cout << "g = " << g << std::endl;   // g = 1 +x   (unaffected)
```
...
**`qbpp::toExpr()`** also accepts an array, returning an array of expressions with the same shape, e.g. `auto f = qbpp::toExpr(qbpp::var("x", 3));`. This is required by operations that are defined only for arrays of expressions, such as division by an integer — see [Basic Operators and Functions for Arrays](OPVECTOR).
...
| `DOUBLE_TYPE_C64E128` | 128-bit integer solver (higher precision, recommended) |
| `DOUBLE_TYPE_C128E128` | 128-bit integer solver (widest dynamic range) |
...
Start with the default `DOUBLE_TYPE`. For most models it keeps essentially all of the precision of the
original `double` coefficients.

Precision runs out when **the spread between your smallest and largest coefficients is large compared to
the size of the whole model**. Consider `DOUBLE_TYPE_C64E128` when:
...
`DOUBLE_TYPE_C64E128` keeps far more coefficient precision while **searching at essentially the same speed
as `DOUBLE_TYPE`**; it uses somewhat more memory. `DOUBLE_TYPE_C128E128` handles a wider dynamic range
still, but reaches worse solutions in the same time, so use it only when coefficients do not fit with
`DOUBLE_TYPE_C64E128`.
