...
[Native integer variables](NATIVE_INTEGER) (`qbpp::int_var()`) can also be used as keys.
The value must lie within the declared range; an out-of-range value throws `std::invalid_argument`.
See [Native integer variables](NATIVE_INTEGER) for an example.
