...
1. **Register a function** — `solver.set_callback(fn)`. `fn` is called as `fn(solver)`, receiving the solver itself. No inheritance is involved, which makes it convenient for an inline lambda or for a callback that has to live in an existing class hierarchy
2. **Subclass** — derive from `ABS3Solver` and override the `callback()` virtual method

If both are used, the override wins and the registered function is not called. Call `ABS3Solver::callback()` from the override to run both.
...
int main() {
  ...
  qbpp::ABS3Solver solver(f);
  solver.set_callback([](const qbpp::ABS3Solver& s) {
    if (s.event() == qbpp::CallbackEvent::Start) {
      s.timer(1.0);                       // enable a timer every second
    } else if (s.event() == qbpp::CallbackEvent::BestUpdated) {
      std::cout << "New best: energy=" << s.best_sol().energy()
                << " TTS=" << s.best_sol().tts() << "s" << std::endl;
    }
  });
...
Call `solver.set_callback(nullptr)` to unregister.
