...
Coefficients can also be **`double`**: in C++ define `DOUBLE_TYPE` (or `DOUBLE_TYPE_C64E128` for higher
precision); in Python `import pyqbpp.d` (or `pyqbpp.dc64e128`). The energy is then returned as a `double`.
...
print("f =", f)     # f = 1 +x +y
print("g =", g)     # g = 1 +x +y   (also changed)
```
...
- **More external solver backends**: PyQBPP can dispatch a model to a much wider range of external solvers. In addition to the MILP solvers (SCIP, HiGHS, GLPK, CBC) that are also available from C++, PyQBPP adds the D-Wave samplers (Neal / Tabu / Steepest Descent), OpenJij, TYTAN-SDK MIKAS, qubovert, Simulated Bifurcation, IBM Qiskit Optimization, and Google OR-Tools CP-SAT — each behind the same `solver.search()` protocol. See [QUBO/HUBO Solvers](python/QUBO_HUBO_SOLVERS) and [CP Solvers](python/CP_SOLVERS).
...
| **External solvers** | MILP (SCIP / HiGHS / GLPK / CBC) | All of C++'s, **plus** the D-Wave samplers, OpenJij, TYTAN, qubovert, Simulated Bifurcation, Qiskit, OR-Tools CP-SAT |
