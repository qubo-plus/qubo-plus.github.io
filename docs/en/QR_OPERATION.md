...
## Nonlinear functions: `abs()`, `relu()`, `max()`, `min()`
The absolute value, ReLU, maximum, and minimum can be used directly inside
expressions. The solvers bundled with QUBO++ handle the function values
directly when searching, so there is no need to design auxiliary variables
or penalty polynomials by hand. See
[Nonlinear Functions and Native Constraints](CONSTRAINTS) for details.

- **`qbpp::abs(f)`** / **`qbpp::abs(f, k)`**:
  Returns a `qbpp::Expr` denoting $|f|$ / $|f|^k$ (`k` is 1 or 2).

- **`qbpp::relu(f)`** / **`qbpp::relu(f, k)`**:
  Returns a `qbpp::Expr` denoting $\max(0, f)$ / $\max(0, f)^k$
  (`k` is 1 or 2).

- **`qbpp::max(f, g)`** / **`qbpp::min(f, g)`**:
  Returns a `qbpp::Expr` denoting the maximum / minimum of two
  expressions.

Expressions containing these functions have the following limitations:

- Any constant coefficient works, including negative ones (a negative
  coefficient rewards a large function value). MIP solvers accept
  positive coefficients only.
- `expand_cons()`, `reduce()`, and `binarize()` are not supported.
- The bundled solvers support all of these functions. Among MIP solvers,
  ScipSolver (Quadratic formulation) supports
  `abs` and `relu` (`k` = 1, 2), and linearization-based MIP solvers
  support `abs` and `relu` (`k` = 1). `max` and `min` work on MIP
  solvers in the convex directions (minimizing `w*max`, maximizing min).
