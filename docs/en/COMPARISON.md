...
> **Note**
> This page describes the classic approach of adding a constraint to the
> objective as a penalty expression (a polynomial whose value is 0 exactly
> when the constraint is satisfied). Wrapping the constraint part in
> `qbpp::cons()` instead **declares it as a native constraint**: no auxiliary
> (slack) variables are introduced and the bundled solvers search efficiently
> for solutions that satisfy it, which **often yields better solutions**.
