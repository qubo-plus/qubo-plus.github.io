...
The basic operators **`+`**, **`-`**, and **`*`** work for arrays of variables and expressions. **`/`** (by an integer) works for arrays of expressions (convert a variable array to an expression array first with `qbpp::toExpr`, e.g. `auto f = qbpp::toExpr(x);`).
...
Similarly, the compound operators **`+=`**, **`-=`**, **`*=`**, and **`/=`** work for arrays of expressions (a variable array's element type cannot change in place — convert it with `qbpp::toExpr` first).
