...
QUBO++ supports the following **nonlinear functions** directly inside
expressions:
...
| `qbpp::cons(l <= f <= u)` etc. | squared violation (0 when satisfied) | **declaring constraints** |
...
`qbpp::max(f, g)` and `qbpp::min(f, g)` denote the maximum and minimum
of two expressions. The following program solves the same
...
`100 * qbpp::cons(c1) + 100 * qbpp::cons(c2)`.
...
outside `qbpp::cons()`, and each `qbpp::cons()` wraps exactly one
comparison (a chained two-sided range counts as one; an array comparison
gives one constraint per element).
...
expression is printed. Printing `f.cons()` shows the declared **constraint
list** alone (the weight prefix is omitted when 1, one-sided bounds are
shown one-sided):
...
solvers do not support it), including with the double-coefficient frontend
(`DOUBLE_TYPE*`).
...
Constraint bodies may also contain negated literals `~x` (for example
`~x*~y*~z + w == 1`). External MIP/ILP solvers do not accept nonlinear
constraint bodies; expand them to the classic penalty form with
`expand_cons()` (see below) before handing the expression to those solvers.
...
- Weights are normally positive, but negative weights are also accepted
  ...
  semantics) reports an error.
- Operations that would destroy the constraint declaration — `sqr()`,
  multiplying two expressions, multiplying by zero (the constraints
  would silently vanish), `reduce()`, ... — report an explicit error.
- Nonlinear functions and `cons()` **cannot be nested** — placing a
  function term or a `cons()` inside a function body or inside a
  `cons()` constraint body reports an explicit error (likewise for the
...
The nonlinear functions `abs`, `relu`, `max`, and `min` are supported as
follows:
...
- ScipSolver (Quadratic formulation) supports
  `abs` and `relu` with exponents 1 and 2. Linearization-based MIP
  solvers (SCIP Linearize, HiGHS, CBC, and GLPK) support exponent 1
  only.
- `max` and `min` expand to exact relu identities, so MIP solvers
  handle the convex directions — minimizing $w \cdot \max$ and
  maximizing min ($-w \cdot \min$ inside a minimized objective);
  the non-convex directions raise an explicit error.
- Any constant coefficient works on the bundled solvers, including
  negative ones (a negative coefficient rewards a large function value).
  MIP solvers accept positive coefficients only. Multiplication is
  supported by scalar constants only — multiplying by a variable or an
  expression, and `sqr()`, report an explicit error. Expressions
...
| `ScipSolver`, `HighsSolver`, `CbcSolver`, `GlpkSolver` | **hard**: constraints are passed to the MIP as linear constraints (weights are ignored; an error is reported if any constraint carries a negative weight) |
...
form with the comparison operators can be migrated to native
...
described in [Comparison Operators](COMPARISON)) and by $v^2$ with `qbpp::cons()`. For a violation of 1 the
