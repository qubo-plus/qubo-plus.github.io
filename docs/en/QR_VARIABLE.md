...
| `DOUBLE_TYPE_C64E128` | `double` | `double` | 128-bit integer solver (higher precision, recommended) |
| `DOUBLE_TYPE_C128E128` | `double` | `double` | 128-bit integer solver (widest range) |
...
- Dyadic coefficients (1, 1/2, 1/4, ...) are represented exactly. A coefficient vastly smaller
  ...
  notice; `DOUBLE_TYPE_C64E128` gives a much wider dynamic range.
...
|---|---|---|---|
| Integer variable | `l <= qbpp::var_int("x") <= u` | expanded into binary variables | can be handed to external binary-only solvers |
| [Native integer variable](NATIVE_INTEGER) | `l <= qbpp::int_var("x") <= u` | keeps the integer value as is | quantities solved by the bundled and MIP solvers |
...
A **native integer variable** is a `qbpp::IntVar` that keeps its integer
value as is. It is a distinct type rather than a `qbpp::Expr`, but it is
converted to `qbpp::Expr` automatically when used in an expression, so it
reads just like an ordinary variable. See
[Native Integer Variables](NATIVE_INTEGER) for details.

### Native integer variable creation functions

- **`l <= qbpp::int_var("name") <= u`**:
  Creates a native integer variable `qbpp::IntVar` named `"name"` with the
  range `[l, u]`. The range is mandatory and must satisfy `l < u`.

- **`l <= qbpp::int_var("name", s1, s2, ...) <= u`**:
  Creates an array `qbpp::Array<Dim, IntVar>` of native integer variables
  sharing the range `[l, u]`. Elements are named `name[i]` / `name[i][j]`
  and accessed with `x[i]` / `x[i][j]` (the call forms `x(i)` / `x(i, j)`
  also work).

### Native integer variable member functions
For a native integer variable `x` (`qbpp::IntVar`):

- **`energy_t x.lo()`**: Returns the minimum value `l` of `x`.
- **`energy_t x.hi()`**: Returns the maximum value `u` of `x`.

The value in a solution `sol` is read with `sol(x)` (or `sol.get(x)`) and
written with `sol.set(x, v)`.

### Conversion

- **`qbpp::binarize(f)`**:
  Returns a new `qbpp::Expr` in which the native integer variables of `f`
  are expanded into the binary encoding. MIP solvers handle integer
  variables directly, so this is needed only when handing a model to an
  external QUBO solver that accepts binary variables only.
