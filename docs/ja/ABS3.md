...
1. **関数を登録する** — `solver.set_callback(fn)`。`fn` はソルバー自身を引数として `fn(solver)` の形で呼ばれます。継承が不要なので、ラムダをその場で書きたいときや、既存のクラス階層と組み合わせたいときに便利です
2. **サブクラス化する** — `ABS3Solver` を継承して `callback()` 仮想メソッドをオーバーライドします

両方を指定した場合はオーバーライドが優先されます（登録した関数は呼ばれません）。両方を実行したい場合は、オーバーライドの中で `ABS3Solver::callback()` を呼んでください。
...
登録を解除するには `solver.set_callback(nullptr)` を呼びます。
...
呼び出せる場所：
- **`callback()` の内部（または登録した関数の中）** — `best_sol().energy()` などを観察して停止を判断
