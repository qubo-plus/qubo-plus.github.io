...
```
{% endraw %}
`c * x` は要素ごとの積を2次元の項の配列として返し、`qbpp::sum` がその全要素を合計して単一の `Expr` を返します。このプログラムの出力は以下の通りです:
```
...
```
このプログラムでは、`qbpp::var_int("x", 4) == 0` がプレースホルダとして定数ゼロの整数変数 `qbpp::Expr` 4個の配列を作成します。
...
配列は以下のファクトリ関数で作成します:
- **`qbpp::var("name", s1, s2, ...)`**: 多次元のバイナリ変数の配列。
- **`qbpp::expr(s1, s2, ...)`**: ゼロ初期化された多次元の式の配列。
- **`qbpp::array({v1, v2, ...})`**: 1次元の整数定数の配列。
- **`qbpp::array(s1, s2, ...)`**: ゼロ初期化された多次元の整数配列。
- **`l <= qbpp::var_int("name", s1, s2, ...) <= u`**: 多次元の整数変数の配列。
