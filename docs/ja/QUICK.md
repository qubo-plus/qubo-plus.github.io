...
- **`-Wfloat-conversion`**: 浮動小数点数から整数への暗黙変換を警告します。整数係数のライブラリでは `3.5 * x` や `f <= 2.5` のような実数リテラルが黙って切り捨てられる（それぞれ `3 * x`、`f <= 2`）ため、付けておくことを推奨します。実数係数を扱いたい場合は `#define DOUBLE_TYPE` 版を使ってください。
- **`-ldl`**: 動的ローダライブラリをリンクします（QUBO++ は実行時に `dlopen` で .so をロードします）。
- **`-pthread`**: POSIX スレッドのライブラリをリンクします（QUBO++ の `.so` が `std::thread` を内部で使用するため必須）。
