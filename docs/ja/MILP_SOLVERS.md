...
> `topk_sols` はベストエフォートです。これらのソルバーは
...
1. **関数を登録する** — `solver.set_callback(fn)`。`fn` はソルバー自身を引数として呼ばれます（継承不要）
2. **サブクラス化する** — ソルバーを継承し `callback()` 仮想メソッドを override します

両方を指定した場合はオーバーライドが優先されます。両方実行したい場合はオーバーライドの中で基底クラスの `callback()` を呼んでください。
