...
領域ごとの星の個数を表す式のベクトルを作ります。

3 種類の等式制約と $2\times 2$ の窓の不等式制約は、いずれも `qbpp::cons()` で宣言します:
- `x(i, qbpp::all)` は行 $i$ の $N$ 個の変数、`x(qbpp::all, i)` は列 $i$ の $N$ 個の変数です。
- `x(qbpp::slice(i, i + 2), qbpp::slice(j, j + 2))` は左上が $(i,j)$ の $2\times 2$ の窓の 4 個の変数です。
...
{% raw %}`search({{"target_energy", 0}})`{% endraw %} で目標エネルギー $0$ に到達した時点で探索を打ち切ります。
