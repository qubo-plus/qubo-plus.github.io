...
係数は **`double`** にもできます。C++ では `DOUBLE_TYPE`（高精度が必要なら `DOUBLE_TYPE_C64E128`）を定義し、Python では `import pyqbpp.d`（または `pyqbpp.dc64e128`）をインポートします。エネルギーは `double` で返されます。
...
- **対応する外部ソルバーが多い**: PyQBPP はより多くの外部ソルバーにモデルを渡せます。C++ からも使える MILP ソルバー（SCIP, HiGHS, GLPK, CBC）に加え、D-Wave samplers (Neal / Tabu / Steepest Descent), OpenJij, TYTAN-SDK MIKAS, qubovert, Simulated Bifurcation, IBM Qiskit Optimization, Google OR-Tools CP-SAT に対応します（いずれも同じ `solver.search()` プロトコルで呼べます）。[QUBO/HUBO ソルバー](python/QUBO_HUBO_SOLVERS) と [CP ソルバー](python/CP_SOLVERS) を参照してください。
...
| **外部ソルバー** | MILP（SCIP / HiGHS / GLPK / CBC） | C++ に**加えて** D-Wave samplers, OpenJij, TYTAN, qubovert, Simulated Bifurcation, Qiskit, OR-Tools CP-SAT |
