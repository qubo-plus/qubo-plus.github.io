...
## QUBO++プログラム
以下のQUBO++プログラムは、ランダムに生成した $N=15$ 箇所の場所（デポ＋顧客14）と $V=3$ 台の車両からなるCVRPインスタンスの解を、制限時間10秒で求めます。
ベクトル `locations` は3つ組 `(x,y,d)` を格納し、`(x,y)` は場所の2次元座標、`d` は顧客の需要量です（デポの需要量は0）。
ベクトル `vehicle_capacity` は $V=3$ 台の車両の容量 `{200, 250, 300}` を格納します。

距離は `std::sqrt` で計算した**正確なユークリッド距離**（丸めなし）を用います。デフォルトの `coeff_t` は整数型なので、ヘッダのインクルード前に `DOUBLE_TYPE` を定義して[実数（double）係数](VAREXPR#real-double-coefficients)フロントエンドを使い、`objective` を `double` で構築します。
...
このプログラムは、まずソートした需要量と最大の車両容量からスロット数 `L`（このインスタンスでは9）を計算し、$V\times L\times N = 3\times 9\times 15 = 405$ 個のバイナリ変数配列 `a` を定義します。次に目的関数項 `objective` と制約項 `row_constraint`・`column_constraint`・`capacity_constraint` を定義し、`qbpp::cons()` で制約として宣言して結合します：`f = objective + 3000 * qbpp::cons(row_constraint + column_constraint + capacity_constraint)`。そして Easy Solver を使用して、制限時間10秒で `f` を最小化する割り当て `sol` を探索します。

`f.cons(sol)` は違反している制約の本数（全充足なら0）を返します。実行例として次の結果が得られます：
```
...
最後に、プログラムは得られた解をグラフとして可視化し、`cvrp15.svg` に書き出します：

<p align="center">
  <img src="../images/cvrp15.svg" alt="N=15のCVRPインスタンスの解" width="80%">
</p>
