...
| `DOUBLE_TYPE_C64E128` | 128ビット整数ソルバー（高精度・推奨） |
| `DOUBLE_TYPE_C128E128` | 128ビット整数ソルバー（最も広いダイナミックレンジ） |
...
最大の係数に比べて極端に小さい係数は、スケーリングの精度を下回ると `0` として扱われ、その項は除外されます（エラーではなく短い通知が出ます）。項がすべて消えた変数は目的関数に影響しない自由変数となり、解からの読み出し（`sol(x)`、`sol(x[i])`）は `0` を返します。`sol.has(x[i])` でその変数が残っているか確認できます。`simplify_as_binary` で相殺して消えた変数も同様です。エネルギー範囲の真のオーバーフローは従来どおりエラーになります。

#### どのマクロを選ぶか

既定の `DOUBLE_TYPE` で始めてください。ほとんどのモデルでは、これで元の `double` の精度をほぼそのまま保てます。

精度が足りなくなるのは、**係数の大小の開きがモデル全体の規模に対して大きいとき**です。次のような場合は `DOUBLE_TYPE_C64E128` を検討してください。

- 項数が非常に多い（10^7 以上が目安）
- 制約のペナルティ重みが目的関数の係数より何桁も大きい
- 1 つのモデルの中で係数の大きさが何桁にもわたる

`DOUBLE_TYPE_C64E128` は保持できる係数の精度が大きく上がる一方で、**探索速度は `DOUBLE_TYPE` とほぼ変わりません**。メモリ使用量はやや増えます。`DOUBLE_TYPE_C128E128` はさらに広いダイナミックレンジを扱えますが、同じ時間で到達する解の質が落ちるため、`DOUBLE_TYPE_C64E128` でも係数が収まらない場合に限って使ってください。
