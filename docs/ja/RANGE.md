...
$f$ が2次以上の場合、$g$ はHUBO式になります（$g$ の次数は $f$ の2倍になるため）。
...
変更点は `100 * (c1 + c2)` を `100 * qbpp::cons(c1) + 100 * qbpp::cons(c2)` に書き換えただけです。
...
ペナルティ式では範囲制約ごとに上記の補助変数が必要ですが、
`cons()` で宣言した場合は補助変数を導入しません。
...
MIP ソルバー（SCIP 等）では**ハード制約**（必ず満たすべき制約）として扱われます。
