...
`ABS3Solver` と同じ 2 通りの方法が使えます。どちらも同じイベント・
同じメソッドが使えます。
...
オーバーライドの中で `ABS2Solver::callback()` を呼んでください。
...
コールバック内では次のメソッドが使えます。

- **`best_sol()`** — 現在の最良解（`const qbpp::Sol&`）
- **`event()`** — このコールバックを起こしたイベント
- **`timer(seconds)`** — `Timer` イベントの周期を設定する（`0` で無効）。
  既定では無効なので、通常は `Start` の中で 1 回呼びます
- **`hint(sol)`** — 探索中のソルバーにヒント解を与える
- **`terminate()`** — 探索を協調的に打ち切る
