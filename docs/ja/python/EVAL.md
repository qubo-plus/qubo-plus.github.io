...
## 辞書を使った評価
式の値は、すべての変数への値の割り当てを `{変数: 値}` の辞書として与えることで計算できます。
...
```
このプログラムでは、辞書 `{x: 0, y: 1, z: 1}` で $x=0$, $y=1$, $z=1$ の割り当てを定義しています。
...
**`sol[x] = value`** で個々の変数に値を割り当てることができます。
...
メソッド **`comp_energy()`** はエネルギー値を返します。エネルギーが無効な場合のみ再計算し、既に有効（キャッシュ済み）であれば再計算せずにキャッシュ値を返します。
