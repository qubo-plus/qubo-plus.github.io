...
> 予告なく変更される可能性があります。Gurobi（C++ からも利用可）を除き、本ページの
...
- **`self.event()`** — 現在のイベント。`EVENT_*` 定数（`EVENT_START`=0, `EVENT_BEST_UPDATED`=1, `EVENT_TIMER`=2）と比較できる整数です
...
`beta_schedule_type`, `seed`。`DWaveSolver` と同様、`time_limit` は非対応で
degree ≤ 2 が必要。
