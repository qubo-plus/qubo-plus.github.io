...
nav_order: 36
lang: ja
hreflang_alt: "en/python/PARTITION_FORMULATIONS"
hreflang_lang: "en"
---

# 分割問題の3通りの定式化

PyQBPP では、同じ問題を従来の **2 乗ペナルティ**（`sqr`）、
...
| 定式化 | エネルギー | モデル | 特徴 |
|---|---|---|---|
| `qbpp.sqr(p - q)` | $(P - Q)^2$ | 2 次式に展開（$n(n+1)/2$ 項） | 最も汎用的。外部の QUBO ツールでも扱える |
| `qbpp.abs(p - q)` | $\lvert P - Q \rvert$ | 展開なし（$n$ 項の本体） | エネルギーが差そのもの |
| `qbpp.cons(p - q == 0)` | $(P - Q)^2$ | 展開なし（$n$ 項の本体） | 制約として宣言。違反の集計・`target_energy` に参加 |
