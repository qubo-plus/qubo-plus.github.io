...
nav_order: 26
lang: ja
hreflang_alt: "en/python/TIPS"
hreflang_lang: "en"
---

# 高速化のための Tips

このページでは、効率的な PyQBPP プログラムを書くための一般的な注意点とベストプラクティスを紹介します。
PyQBPP は求解の前に式を記号的に構築するため、式の構築方法がパフォーマンスに大きく影響します。
...
これは Python 標準の動作と一致しています — `list += [x]` はその場で変更しますが、
`list = list + [x]` は新しいリストを作成します。
...
# ❌ 遅い: Python/C 境界を毎回越えて逐次的に式を構築する
f = 0
for i in range(n):
    f += x[i]

# ✅ 速い: 全要素の総和を共有ライブラリ内の単一呼び出しで実行
f = qbpp.sum(x)
```
...
PyQBPP は Array 同士および Array とスカラーの演算をサポートしています。
...
penalty = qbpp.sqr(diff)    # sqr を Array 全体に適用
```
