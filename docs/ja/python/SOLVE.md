...
1. ソルバーオブジェクト（**`qbpp.EasySolver`** または **`qbpp.ExhaustiveSolver`**）を作成します。
...
`qbpp/easy_solver.hpp` のインクルードが必要なC++版と異なり、PyQBPPでは `EasySolver` がトップレベルクラスとして公開されているため、`import pyqbpp as qbpp` のみで利用可能です。
...
`EasySolver` と同様、`import pyqbpp as qbpp` 以外の追加インポートは不要です。
...
`import pyqbpp as qbpp` 以外の追加インポートは不要です。
...
パラメータ、コールバック、複数解の収集、ヒント解の詳細については **[Easy Solver](EASYSOLVER)**、**[Exhaustive Solver](EXHAUSTIVE)**、**[ABS3 Solver](ABS3)** をご覧ください。
