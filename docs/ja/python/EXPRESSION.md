...
| `pyqbpp.ExprExpr` | 制約式 | 比較演算子や範囲演算子が生成する (penalty, body) のペア |

どちらも算術文脈では `Expr` に decay するため、多くの場合はそのまま式として扱えます。
...
明示的に空の `Expr` が必要な場合（例えば `int` を初期値にせずホットループでアキュムレータを始める場合）は、ファクトリ関数 `qbpp.expr()` を使用してください。
...
`VarInt` は算術文脈で **`Expr` に decay** するため、式が期待される場所でそのまま使用できます:
```python
...
`VarInt` と同様、`ExprExpr` は算術文脈で **`Expr` (penalty 部分) に decay** します:
```python
f = c1 + c2 + qbpp.sqr(x - 4)        # ExprExpr と Expr を自由に組み合わせられる
f.simplify_as_binary()
```
評価前の式にアクセスするには `c.body` を使います（例えば解を得たあとに `sol(c.body)` で実値を確認）。詳細と対応する比較構文の一覧は [比較演算子](COMPARISON) を参照してください。
