...
Pythonのアンパック演算子 `*` を使えば、`qbpp.array()` ファクトリ内で `concat()` を置き換えられます:
...
`qbpp.concat([1, x, 0])` で拡張ベクトルを構築します:
...
1. **`x`** は $(n{-}1) \times n$。`qbpp.concat([ones, x, zeros], axis=0)` で `axis=0` に沿ってガード行を追加すると $(n{+}1) \times n$ となり、各列がドメインウォール。`xg[:n] - xg[-n:]`（先頭 $n$ 行と末尾 $n$ 行の差）で $n \times n$ の行列 `x_oh` を得て、各**列**がone-hotになります。

2. **`y`** は $n \times (n{-}1)$。`qbpp.concat([ones, y, zeros], axis=1)` で `axis=1` に沿ってガードビットを追加すると $n \times (n{+}1)$ となり、各行がドメインウォール。`yg[:, :n] - yg[:, -n:]`（先頭 $n$ 列と末尾 $n$ 列の差）で $n \times n$ の行列 `y_oh` を得て、各**行**がone-hotになります。
...
- **`qbpp.concat([1, x, 0], axis=0)`**: 全1のガード行を上に追加。
