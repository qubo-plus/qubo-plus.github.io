...
## 変数配列と整数変数の作成
`var()`関数は、変数の配列や整数変数を作成するために`shape=`および`between=`キーワード引数をサポートしています:
```python
import pyqbpp as qbpp

x = qbpp.var("x")                              # 単一のバイナリ変数
v = qbpp.var("v", shape=3)                      # 1次元配列: v[0], v[1], v[2]
v = qbpp.var("v", shape=(3,))                   # 同じ（タプル形式も可）
m = qbpp.var("m", shape=(2, 3))                 # 2次元配列 (2x3行列)
y = qbpp.var("y", between=(0, 10))              # [0, 10]の範囲の整数変数
w = qbpp.var("w", shape=(3, 4), between=(0, 7)) # [0, 7]の範囲の整数変数の2次元配列
e = qbpp.expr(shape=(2, 3))                     # 2次元の式配列 (全てゼロ)
```
