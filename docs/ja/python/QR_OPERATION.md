...
| 比較（範囲）                    | `qbpp.constrain(f, between=(l, u))`                             | Global          | `ExprExpr`        | `ExprType`-`int`-`int`   |
...
| 評価                           | `f(ml)`                                               | Global          | `int`             | `Expr`-`dict`            |
| 置換                           | `qbpp.replace(f, ml)`                                      | Global          | `Expr`            | `ExprType`-`dict`        |
| 置換                           | `f.replace(ml)`                                       | In-place        | `Expr`            | `dict`                   |
...
PyQBPPでは、代わりに `constrain()` 関数を使用します。
```python
...
`pyqbpp.Expr` オブジェクト `f` と辞書 `ml` に対して、評価関数 `f(ml)` は `ml` で指定された変数の割り当ての下で `f` の値を評価し、結果の整数値を返します。
...
## 置換関数: `replace()`
`replace()` 関数は `{変数: 式}` の辞書を受け取ります。式には整数値も指定できます。

`pyqbpp.Expr` オブジェクト `f` と辞書 `ml` に対して:
