...
整数値をそのまま保持する[ネイティブ整数変数](NATIVE_INTEGER)（`IntVar`）は
`Expr` とは別の型で、リファレンスは[変数と式](QR_VARIABLE)にあります。
...
制約を `qbpp.cons()` で作成すると、**ネイティブ制約として宣言**された式になります。
...
| 構文 | 意味 |
|---|---|
| `qbpp.cons(f, equal=n)` | 等式制約 `f == n` |
| `qbpp.cons(f, equal=[a, b, ...])` | 離散許容値集合（`f` が `a`, `b`, ... のいずれか） |
| `qbpp.cons(f, between=(l, u))` | 範囲制約（片側は `None` で無制限） |
| `qbpp.cons(制約式)` | `==` で作った制約式をネイティブ制約として宣言 |
| `qbpp.cons(配列の比較)` | **要素ごとに1本**の制約 |
...
| `sol(f)` | `int` | ソルバーが報告する Energy と一致（目的関数＋ペナルティ） |
| `f.cons(sol)` | `int` | 解 `sol` で違反している制約の本数（0 なら全充足） |
| `f.cons()` | `str` | 宣言済み制約リストの文字列（`print(f.cons())` で表示） |
| `f.violations(sol)` | `list` | 各制約の値・境界・違反量・重みを報告 |
| `f.is_feasible(sol)` | `bool` | 全制約を充足しているか |
| `f.simplify_as_binary()` | `Expr` | 目的関数と制約の両方を簡約、**宣言は維持** |
| `qbpp.replace(f, ml)` | `Expr` | 変数置換、宣言は維持 |
| `qbpp.expand_cons(f)` / `f.expand_cons()` | `Expr` | 従来のペナルティ式に展開（宣言は消える） |
...
| `qbpp.cons(...)` | `Expr` (制約宣言) | ネイティブ制約として宣言 |
| `qbpp.expand_cons(f)` | `Expr` | 宣言された制約をペナルティ式に展開 |
