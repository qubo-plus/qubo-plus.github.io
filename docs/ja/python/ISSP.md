...
不等式制約 `(0 <= total) & (qbpp.same <= T)` は `constraint` に格納されます。PyQBPPでは、このような制約は内部的に非負のペナルティ項に変換され、制約が満たされると0になります。
...
不等式制約 `constraint2 = (0 <= total) & (qbpp.same <= T)` は、選択された合計が `T` を超えないことを保証します。
