...
```
この PyQBPP プログラムでは、制約 $fa(a,b,i,o,s)$ は `... == 0` を使って実装されており、直感的に制約 $a+b+i=2o+s$ を表しています。
...
出力で `z` と固定したキャリービットが `0` と表示される点に注意してください。`replace()` は固定した変数をモデルから消去するため、`sol(z)` / `sol(c[4])` は割り当てた `1101` / `1` ではなくデフォルト値を返します。実際に解かれている変数は `x` と `y` です。
