...
print()
```
`qbpp.var("x", shape=N)` によって生成された `N = 16` 個のバイナリ変数のベクトル `x` に対して、上記のQUBO定式化に従って式 `objective`、`constraint`、`f` が構築されます。ここで `constraint` は `0` を初期値として辺に関するペナルティ和を蓄積し、`qbpp.sum(x)` は `x` のすべての要素の和を計算します。続いて `f.simplify_as_binary()` により、バイナリ(0/1)ルールに基づく同類項のマージなどの簡約が in-place に適用されます。
