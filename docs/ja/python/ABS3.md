...
より細かい制御が必要な場合は、次の 2 つの方法があります。どちらも同じイベント・同じメソッドが使えます。

1. **関数を登録する** — `solver.set_callback(fn)`。`fn` はソルバー自身を引数として `fn(solver)` の形で呼ばれます。継承が不要なので、既存のクラス階層やミックスインと組み合わせたいときはこちらを使ってください
2. **サブクラス化する** — `ABS3Solver` を継承して `callback()` メソッド（引数なし）をオーバーライドします

両方を指定した場合はオーバーライドが優先されます（登録した関数は呼ばれません）。両方を実行したい場合は、オーバーライドの中で `super().callback()` を呼んでください。
...
`callback()` の中（および登録した関数が受け取ったソルバー `s` に対して）では、以下のメソッドが利用可能です:
...
def on_event(s):
    if s.event() == 0:        # Start
        s.timer(1.0)          # 1秒ごとのタイマーコールバックを有効化
...
引数は関数でなくても、呼び出し可能なオブジェクト（`__call__` を持つクラスのインスタンス）であれば何でも渡せます。状態を持たせたい場合に便利です:
...
登録を解除するには `solver.set_callback(None)` を呼びます。
...
呼び出せる場所:
- **`callback()` の内部（または登録した関数の中）** — `self.best_sol().energy` などを観察して停止を判断
