...
基本的な使い方は以下の通りです。
- `var("name", shape=(s1, s2, ..., sd))`: 指定された `name` と形状 $s_1\times s_2\times \cdots\times s_d$ を持つ `Var` オブジェクトの配列を作成します。
- `var("name", shape=(s1, s2, ..., sd), between=(l, u))`: 指定された範囲と形状を持つ `VarInt` オブジェクトの配列を作成します。
...
for i in range(len(max_vals)):
    print(f"x[{i}] = {x[i]}")
```
ここで、`qbpp.var("x", shape=4, equal=0)` は定数値0で初期化された `VarInt` プレースホルダ4個の可変配列を作成します。
...
- **`expr(shape=(s1, s2, ..., sd))`**: 形状 $s_1\times s_2\times \cdots\times s_d$ を持つ `Expr` オブジェクトの多次元配列を作成します。
