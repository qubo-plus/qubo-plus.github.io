...
C++ 版の `INTEGER_TYPE_*` マクロに相当する型選択を、PyQBPP ではインポートするサブモジュール名（`pyqbpp.c32e64`、`pyqbpp.cppint` など）で指定します。以降の `qbpp.var`, `qbpp.constrain`, `qbpp.EasySolver` などの呼び出しは、選択した係数・エネルギー型を一貫して使用します。
