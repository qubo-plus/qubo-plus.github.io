...
**否定リテラルを含む HUBO (nHUBO)** は、否定リテラルを含む項も持つことができる HUBO です。
...
この展開により項数は大幅に増加します。
一般に、$n$ 個の否定リテラルを含む項を展開すると、$2^n$ 個の項が生成されます（項が否定リテラルのみで構成される場合は定数項も含まれます）。

QUBO++ は nHUBO モデルを作成できます。
QUBO++ に同梱された3つのソルバーはすべて、通常の HUBO モデルへ変換することなく、否定リテラルをネイティブに処理します。
