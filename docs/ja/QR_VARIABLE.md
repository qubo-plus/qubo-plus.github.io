...
| `DOUBLE_TYPE_C64E128` | `double` | `double` | 128ビット整数ソルバー（高精度・推奨） |
| `DOUBLE_TYPE_C128E128` | `double` | `double` | 128ビット整数ソルバー（最も広いレンジ） |
...
- 2進小数の係数（1, 1/2, 1/4, ...）は厳密に表現されます。最大係数よりはるかに小さい係数は
  スケーリング精度を下回ると通知付きでドロップされます。`DOUBLE_TYPE_C64E128` を使うと
  ダイナミックレンジが大幅に広がります。
...
| [ネイティブ整数変数](NATIVE_INTEGER) | `l <= qbpp::int_var("x") <= u` | 整数値をそのまま保持 | 同梱ソルバー・MIP ソルバーで数量を扱う |
...
- **`l <= qbpp::int_var("name", s1, s2, ...) <= u`**:
  同じ範囲`[l, u]`を持つネイティブ整数変数の配列 `qbpp::Array<Dim, IntVar>` を
