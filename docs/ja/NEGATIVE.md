...
否定リテラルを含む項も持つことができる HUBO を、**否定リテラルを含む HUBO (nHUBO)** と呼びます（[HUBO と QUBO](HUBO_QUBO) を参照）。
QUBO++に付属するソルバーは、このような nHUBO モデルを、正リテラルのみの通常の HUBO 式に展開することなく直接受け付けます。
一方、従来のツールは $\bar{x}$ の代わりに $1-x$ を使用する必要があります。
そのため、多くの否定リテラルを含む項を持つ nHUBO モデルでは、QUBO++は従来のツールより高い性能を発揮できます。
