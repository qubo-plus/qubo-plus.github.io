...
`search()` の呼び出しは、最良の解を格納する `qbpp::EasySolverSol` オブジェクト `sols` を返します。
`qbpp::EasySolverSol` は格納された最良エネルギー解へのイテレータアクセス（`begin()`、`end()`、`cbegin()`、`cend()`）を提供するため、範囲ベースのforループで出力できます。
