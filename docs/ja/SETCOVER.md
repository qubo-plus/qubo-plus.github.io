...
```
このプログラムでは、すべての `j` に対して式 `c[j] >= 1` が要素ごとに作成され、その和が `constraint` に格納されます。各要素の補助整数変数はその要素自身の `pos_sum()` (= `j` を含むサブセット数) に自動的に合わせられるので、上限を手動で指定する必要はありません。

> **注意**.
> 項 `c[j] >= 1` は内部的に補助バイナリ変数を導入する場合があります。
