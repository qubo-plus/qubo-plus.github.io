...
| **その他のライセンス** | 必要 | 個別に定めます | 個別に定めます | 個別に定めます |
...
- **その他のライセンス**: 有効期間・変数数の上限を個別に定めたライセンスを発行する場合があります。適用される条件は、ライセンス発行時にお知らせします。
...
- **バイナリ変数**: 1 個につき 1
- **[ネイティブ整数変数](NATIVE_INTEGER)**: 範囲（上限 − 下限）を $r$ とすると、
...
ライセンスの変数数が Unlimited と表示されている場合は、ライセンスによる
変数数の制限がないことを意味します。Trial の 10,000 や Fallback の 100 の
ような数え方による上限はかかりません。

ただし、QUBO++ の実装上の容量は残ります。バイナリ変数・
[ネイティブ整数変数](NATIVE_INTEGER)は**それぞれ最大 $2^{30}-1$ =
...
> **本節の対象:** 以下のアクティベーションフロー、および [ライセンス状態の確認](#ライセンス状態の確認)・[ライセンスのディアクティベーション](#ライセンスのディアクティベーション) は、**ノードロックライセンス** (単一マシン用キー) を対象としています。アクティベーションはキーを特定の物理マシンに紐づけます。フローティングライセンスについては、本ページ末尾の [フローティングライセンス](#フローティングライセンス) を参照してください。
>
...
- 同じマシンがアクティベート済みと認識されなくなり（例: WSL2 などでネットワークアダプタが変わった場合）、`qbpp-license -k KEY -a` がアクティベーション数の上限に達したと表示される場合でも、そのマシンで `qbpp-license -k KEY -d` を実行すれば、このマシンの既存のアクティベーションを解除できます。その後、改めて `qbpp-license -k KEY -a` を実行してください。この場合も 24時間のクールダウンが適用されます。
- `qbpp-license -d` が解除するのは、実行したマシン自身のアクティベーションだけです。別のマシンのアクティベーションを解除するには、[User Portal](https://qubo-plus.github.io/portal/) で解除するマシンを選んでください。
...
Options:
  ...
  -k, --key KEY       ライセンスキーを指定（-a/-d なしでは情報表示のみ）
  -a, --activate      このマシンにライセンスを登録（ノードロック: アクティベート
                      ＋既定キーとしてキャッシュ、フローティング: キャッシュのみ）
  -d, --deactivate    このマシンからライセンスを撤去（ノードロック: ディアクティ
                      ベート＋キャッシュ削除、フローティング: キャッシュ削除のみ）
